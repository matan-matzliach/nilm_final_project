...
class Device():
    ...
    def __init__(self,current,current_THD,start_time_arr=[],end_time_arr=[],phase=[1,0,0],phase_static=False):
        ...
        self.name='temporal_name'#the device name to be shown in the list initialized to "temporal_name" until the interaction with the user
    ...
    def set_name(self,_name):
        #set the name of the device to add to the device list
        self.name=_name
    def tojson(self):
        #jsonify the device to send it's representation to the app
...
#a procedure that runs every 5 minutes and also every time we add a device to the list or enter the add device activity in the app
    ...
    for phase_ind in range(len(UD_arr)):    
        for ele in UD_arr[phase_ind]: #UD1 / UD2 / UD3
            ...
            if(max_grade>max_grade_arr[phase_ind]):
                ...
                stored[0]=dev1.start_time_arr
                stored[1]=dev1.end_time_arr
...
#this function adds device to the list in the server
def add():
    #ADD
    inpdev=request.get_json()#jsonify the post request sent from the appp
    a=0
    s=inpdev['name']#retreuve device's name
    for dev in list_devices:#loop to check if there is already a device with that name in the device list
        if(inpdev['name']==dev.name):#if statement to check if there is already a device with that name in the device list
            a=1
    if a==0:#add device
        dev1=Device(current=float(inpdev['current']),start_time_arr=stored[0],end_time_arr=stored[1],current_THD=float(inpdev['current_THD']),phase=[int(inpdev['phase0']),int(inpdev['phase1']),int(inpdev['phase2'])])
    ...
    else:#ask user for a different device name
        inpdev['name']=s+str(0)
        return jsonify(inpdev)    
@app.route('/remove_device_json',methods=['GET','POST'])
#this function removes devices from the list in the server when the user asks wants to remove a device
def remove():
    #REMOVE
    inpdev=request.get_json()#jsonify the post request sent from the appp
    dev='a'
    ind=0
    for i in range(len(list_devices)):#a loop to check if the device is in the list
        if list_devices[i].name==inpdev['name']:
            dev=list_devices[i]
            ind=i
    if(dev!='a'):#remove the device
        for j in range(ind,len(device_index)):
            device_index[j]-=1
        list_devices.remove(dev)
        device_index.remove(device_index[ind])
        return jsonify({'name':inpdev['name'],'success':'yes'})#tell the user the device was removed
    return jsonify({'name':inpdev['name'],'success':'no'})#tell the user the device was not found in the list
@app.route('/nilmgraph/I',methods=['GET'])
#a function to recieve the current graph from the server
def show_i():
    return render_template("nilmgraph_I.html")
@app.route('/get_device_list' , methods=['GET'])
#a function to allow the user to ask for the device list and send it to the app
def get_device_list():
    return jsonify([i.tojson() for i in list_devices])
@app.route('/nilmgraph/I THD',methods=['GET'])
#a function to recieve the current thd graph from the server
def show_i_thd():
    return render_template("nilmgraph_I_THD.html")
@app.route('/nilmgraph/P_KVAR',methods=['GET'])
#a function to recieve the poower in kvar graph from the server
def show_p_kvar():
    return render_template("nilmgraph_P_KVAR.html")
@app.route('/nilmgraph/P_KW',methods=['GET'])
#a function to recieve the poower graph in kw from the server
def show_p_kw():
    return render_template("nilmgraph_P_KW.html")
@app.route('/nilmgraph_power',methods=['POST'])
#a function to draw power graph graph
...
#a function to draw the powerper device graph graph
def graph_form_post_devices():
    print(UD1)
    print(UD2)
    print(UD3)
    r=request.get_json()#jsonify the post request sent from the appp
...
#a function to draw cost graph graph
...
if __name__ == "__main__":
    ...
    #plot_graph("Administrato_output1_table1",dictionary_2days,["I1 THD","I2 THD","I3 THD"],True,init_index=start,fin_index=end,stringo="I_THD")
    #plot_graph("Administrato_output1_table4",dictionary_2days,["I1 THD","I2 THD","I3 THD"],True,init_index=start,fin_index=end,stringo="I_THD")
    
    #plot_graph("Administrato_output1_table%d"%tablenum,dictionary_2days,["I1","I2","I3"],True,init_index=start,fin_index=end,stringo="I")
    #plot_graph("Administrato_output1_table1",dictionary_2days,["I1","I2","I3"],True,init_index=start,fin_index=end,stringo="I")
    #plot_graph("Administrato_output1_table4",dictionary_2days,["I1","I2","I3"],True,init_index=start,fin_index=end,stringo="I")
    
    print(dictionary_2days["Administrato_output1_table%d"%tablenum].keys())
    #plot_graph("Administrato_output1_table%d"%tablenum,dictionary_2days,["Power1"],True,init_index=start,fin_index=end,stringo="Power")
