    ...
    for phase_ind in range(len(UD_arr)):    
        for ele in UD_arr[phase_ind]: #UD1 / UD2 / UD3
            ...
            if(max_grade>max_grade_arr[phase_ind]):
                ...
                if(stored[0]!=dev1.start_time_arr and stored[1]!=dev1.end_time_arr):
                    stored[0]=dev1.start_time_arr
                    stored[1]=dev1.end_time_arr
                    ele.append(len(list_devices))
...
def remove():
    ...
    if(dev!='a'):#remove the device
        ...
        p=device_index[-1]
        device_index.remove(device_index[-1])
        for phase_ind in range(len(UD_arr)):    
            for ele in UD_arr[phase_ind]: #UD1 / UD2 / UD3
                if ele[-1]>p:
                    ele[-1]=ele[-1]-1
...
def graph_form_post_cost():
    ...
    tablenum=2
    #for dev in list_devices:
       # print(dev.start_time_arr)
      #  print(dev.end_time_arr)
     #   print("-----------------------------\n")
    #print(months)
    #print(device_index)
    curindxs=[]
    ...
    for i in devsplot:
        for j in range(len(list_devices)):
            if(i==list_devices[j].name):
                curindxs.append(j)
    #print(curindxs)
    plotTotalCost(list_devices,curindxs, price=price, currency_name=currency, months=months)
    #print()
    #print("Events phase 1: ")
    #print_tensor(UD1)
    #print("Events phase 2: ")
