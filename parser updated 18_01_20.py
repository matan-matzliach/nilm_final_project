...
def diff_all_data(dictionary,difdictionary):
    l=list(dictionary.keys())
    for i in range(len(l)):
        for j in l[i+1:]:
            if(l[i][-2]==j[-2]):
                print(l[i]+" and "+j)
                difdictionary[l[i]+" and "+j]=find_difference_between_Data_in_tabels(l[i],j,64)
                sumdictionary[l[i]+" and "+j]=find_sum_of_Data_in_tabels(l[i],j,64)
...
def find_difference_between_Data_in_tabels(table1,table2,num_of_samples):#num_of_samples<=64
    l=[]
    index1,index2=sync_voltage(table1,table2,num_of_samples)
    ...
    return l
def find_sum_of_Data_in_tabels(table1,table2,num_of_samples):#num_of_samples<=64
    l=[]
    index1,index2=sync_voltage(table1,table2,num_of_samples)
    adjusted_row1=dictionary[table1][0]['Data'][index1:index1+num_of_samples]
    adjusted_row2=dictionary[table2][0]['Data'][index2:index2+num_of_samples]
...
if __name__ == "__main__":
    ...
    diff_all_data(dictionary,difdictionary)
